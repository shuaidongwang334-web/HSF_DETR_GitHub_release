...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg='yolov8n.yaml', ch=3, nc=None, verbose=True):  # model, input channels, number of classes
        ...
        # Build strides
        m = self.model[-1]  # Detect() or RTDETRDecoder
        if isinstance(m, RTDETRDecoder):
            self.stride = torch.Tensor([32])  # RT-DETR uses a fixed default stride; avoid slow dummy forward.
        elif isinstance(m, (Detect, Segment, Pose)):
            s = 256  # 2x min stride
...
def parse_model(d, ch, verbose=True, warehouse_manager=None):  # model_dict, input_channels(3)
    """Parse an RT-DETR YAML dictionary into a PyTorch model.

    This minimal parser keeps the original Ultralytics RT-DETR blocks and the
    three HSF-DETR custom modules only: ETB, HCFM and GCConv.
    """
    import ast
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):
        t = m
        m = getattr(torch.nn, m[3:]) if isinstance(m, str) and 'nn.' in m else globals()[m]
        ...
        base_modules = {
            ...
            nn.Conv2d, nn.ConvTranspose2d, DWConvTranspose2d, C3x, RepC3,
            ConvNormLayer, GCConv,
        }
